...
import processingCart from "../cart/processingCart.js";
...
const ProductDetails = (slug) => {
  ...
  const productImg = `.${product.image.desktop}`;
  const productGalleryFirst = `.${product.gallery.first.desktop}`;
  const productGallerySecond = `.${product.gallery.second.desktop}`;
  const productGalleryThird = `.${product.gallery.third.desktop}`;
  ...
  const content = `
    <div class="container">
      <header class="header-product-details">
        ...
        <ul id="menu-tablet" class='menu ${isDesktop ? "hidden" : ""}'>
          <li class="category-item">
            ...
            <h3>HEADPHONES</h3>
            <a href="${URL}/headphones" class="btn-secondary"><span>SHOP</span> <span class="color-main"><svg width="8" height="12" xmlns="http://www.w3.org/2000/svg"><path d="M1.322 1l5 5-5 5" stroke="#D87D4A" stroke-width="2" fill="none" fill-rule="evenodd"/></svg></span></a>
          </li>
          <li class="category-item">
            <img src=${imgThumbnailSpeaker} alt="Speakers">
            <h3>SPEAKERS</h3>
            <a href="${URL}/speakers" class="btn-secondary"><span>SHOP</span> <span class="color-main"><svg width="8" height="12" xmlns="http://www.w3.org/2000/svg"><path d="M1.322 1l5 5-5 5" stroke="#D87D4A" stroke-width="2" fill="none" fill-rule="evenodd"/></svg></span></a>
          </li>
          <li class="category-item">
            <img src=${imgThumbnailEarphone} alt="Speaker">
            <h3>EARPHONES</h3>
            <a href="${URL}/earphones" class="btn-secondary"><span>SHOP</span> <span class="color-main"><svg width="8" height="12" xmlns="http://www.w3.org/2000/svg"><path d="M1.322 1l5 5-5 5" stroke="#D87D4A" stroke-width="2" fill="none" fill-rule="evenodd"/></svg></span></a>
          </li>
          <li class="menu-contact">
            <a href="${URL}/contact" class="button-2">Contact</a>
          </li>
        </ul>
      ...
      <main>
        <section class="product-details">
          ...
          <div class="product-details-included">
            <h2>IN THE BOX</h2>
            <ul>
              ${product.includes
                .map(
          ...
          <div class="product-details-others">
            ...
            <div class="product-details-others-items">
            ${product.others
              .map((item) => {
                const otherImg = `.${item.image.desktop}`;
                return `<div class="product-details-others-item">
                        <img src="${otherImg}" alt="${item.name}">
                        <h3>${item.name}</h3>
                        <a href="${URL}/${product.category}/${item.slug}" class="button-1">SEE PRODUCT</a>
                      </div>`;
              })
              .join("")}
              </div>
        ...
        <section class="about">
          <div class="about-text">
            <h2>Bringing you the <span class="color-main">best</span> audio gear</h2>
...
const initFuncProductDetails = (slug) => {
  addToCart(slug);
  handleDetectScreenChange();
  HandleMenuButton();
  processingCart();
};
export { ProductDetails, initFuncProductDetails };
